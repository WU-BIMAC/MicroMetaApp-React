...
class MicroMetaAppReact extends _react.default.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
    ...
    this.toolbarRef = /*#__PURE__*/_react.default.createRef();
    this.canvasRef = /*#__PURE__*/_react.default.createRef();
    this.settingsMainViewRef = /*#__PURE__*/_react.default.createRef();
    /**
    ...
    this.handleCompleteLoadComponents = this.handleCompleteLoadComponents.bind(this);
    this.handleLoadSettings = this.handleLoadSettings.bind(this);
    this.handleCompleteLoadSettings = this.handleCompleteLoadSettings.bind(this);
    this.handleLoadDimensions = this.handleLoadDimensions.bind(this);
    this.handleCompleteLoadDimensions = this.handleCompleteLoadDimensions.bind(this);
    this.handleLoadTierList = this.handleLoadTierList.bind(this);
    this.handleCompleteLoadTierList = this.handleCompleteLoadTierList.bind(this);
    ...
    this.applyPreviousVersionModificationToMicroscope = this.applyPreviousVersionModificationToMicroscope.bind(this);
    this.applyPreviousModelVersionModificationToMicroscope = this.applyPreviousModelVersionModificationToMicroscope.bind(this);
    this.applyPreviousAppVersionModificationToMicroscope = this.applyPreviousAppVersionModificationToMicroscope.bind(this);
    this.applyPreviousVersionModificationToSetting = this.applyPreviousVersionModificationToSetting.bind(this);
    this.applyPreviousModelVersionModificationToSetting = this.applyPreviousModelVersionModificationToSetting.bind(this);
    this.applyPreviousAppVersionModificationToSetting = this.applyPreviousAppVersionModificationToSetting.bind(this);
    this.createOrUseMicroscope = this.createOrUseMicroscope.bind(this);
    this.createNewMicroscopeFromScratch = this.createNewMicroscopeFromScratch.bind(this);
    this.createOrUseMicroscopeFromDroppedFile = this.createOrUseMicroscopeFromDroppedFile.bind(this);
    this.createOrUseMicroscopeFromSelectedFile = this.createOrUseMicroscopeFromSelectedFile.bind(this);
    //this.setMicroscopeScale = this.setMicroscopeScale.bind(this);

    this.createOrUseSetting = this.createOrUseSetting.bind(this);
    this.createNewSettingFromScratch = this.createNewSettingFromScratch.bind(this);
    this.createOrUseSettingFromDroppedFile = this.createOrUseSettingFromDroppedFile.bind(this);
    this.createOrUseSettingFromSelectedFile = this.createOrUseSettingFromSelectedFile.bind(this);
    ...
    this.handleExportMicroscopeImage = this.handleExportMicroscopeImage.bind(this);
    ...
    this.simulateClickLoadMicroscopeFromPortal = this.simulateClickLoadMicroscopeFromPortal.bind(this);
    this.simulateClickHardwareModeFromPortal = this.simulateClickHardwareModeFromPortal.bind(this);
  ...
  setCreateNewMicroscope() {
    this.setState({
      ...
      if ((0, _genericUtilities.isDefined)(this.props.onModeSelection)) this.props.onModeSelection(1);
    });
  ...
  setLoadMicroscope() {
    this.setState({
      ...
      if ((0, _genericUtilities.isDefined)(this.props.onModeSelection)) this.props.onModeSelection(2);
    });
  ...
  createAdaptedSchema(singleSchemaOriginal, activeTier, validationTier) {
    ...
    Object.keys(properties).forEach(propKey => {
      ...
      if (property.type === _constants.string_object || property.type === _constants.string_array && property.items.properties !== null && property.items.properties !== undefined) {
        let newProp = this.createAdaptedSchema(property, activeTier, validationTier);
  ...
  createAdaptedSchemas(validationTier, standType) {
    ...
    Object.keys(schema).forEach(schemaIndex => {
      ...
      let singleSchema = this.createAdaptedSchema(singleSchemaOriginal, activeTier, validationTier);
      if (singleSchema.title === "Instrument") {
  ...
  static checkScalingFactorAndRescaleIfNeeded(modifiedMic, elementData, scalingFactor) {
  ...
  static applyPreviousAppVersionModificationToObj(originalObj, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2) {
    ...
    if (objSchema !== undefined && objSchema !== null) {
      ...
      if (obj.LightPath !== null && obj.LightPath !== undefined) {
        let newLightPath = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(obj.LightPath, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
      ...
      if (obj.ComponentSettings !== null && obj.ComponentSettings !== undefined) {
        ...
        if (compSettings.LightSource !== null && compSettings.LightSource !== undefined) {
          let sett = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(compSettings.LightSource, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.LightSource = sett;
        }
        if (compSettings.CouplingLens !== null && compSettings.CouplingLens !== undefined) {
          let sett = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(compSettings.CouplingLens, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.CouplingLens = sett;
        }
        if (compSettings.LightSourceCoupling !== null && compSettings.LightSourceCoupling !== undefined) {
          let sett = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(compSettings.LightSourceCoupling, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.LightSourceCoupling = sett;
        }
        if (compSettings.ExcitationFilter !== null && compSettings.ExcitationFilter !== undefined) {
          let sett = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(compSettings.ExcitationFilter, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.ExcitationFilter = sett;
        }
        if (compSettings.Dichroic !== null && compSettings.Dichroic !== undefined) {
          let sett = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(compSettings.Dichroic, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.Dichroic = sett;
        }
        if (compSettings.EmissionFilter !== null && compSettings.EmissionFilter !== undefined) {
          let sett = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(compSettings.EmissionFilter, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.EmissionFilter = sett;
        }
        if (compSettings.RelayLens !== null && compSettings.RelayLens !== undefined) {
          let sett = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(compSettings.RelayLens, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.RelayLens = sett;
        }
        if (compSettings.Detector !== null && compSettings.Detector !== undefined) {
          let sett = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(compSettings.Detector, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.Detector = sett;
        }
        if (compSettings.AdditionalSlot_1 !== null && compSettings.AdditionalSlot_1 !== undefined) {
          let setts = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(compSettings.AdditionalSlot_1, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.AdditionalSlot_1 = setts;
        }
        if (compSettings.AdditionalSlot_2 !== null && compSettings.AdditionalSlot_2 !== undefined) {
          let setts = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(compSettings.AdditionalSlot_2, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.AdditionalSlot_2 = setts;
        }
        if (compSettings.AdditionalSlot_3 !== null && compSettings.AdditionalSlot_3 !== undefined) {
          let setts = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(compSettings.AdditionalSlot_3, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.AdditionalSlot_3 = setts;
        }
        if (compSettings.AdditionalSlot_4 !== null && compSettings.AdditionalSlot_4 !== undefined) {
          let setts = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(compSettings.AdditionalSlot_4, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.AdditionalSlot_4 = setts;
        }
        if (compSettings.AdditionalSlot_5 !== null && compSettings.AdditionalSlot_5 !== undefined) {
          let setts = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(compSettings.AdditionalSlot_5, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.AdditionalSlot_5 = setts;
        }
        if (compSettings.AdditionalSlot_6 !== null && compSettings.AdditionalSlot_6 !== undefined) {
          let setts = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(compSettings.AdditionalSlot_6, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.AdditionalSlot_6 = setts;
        }
        if (compSettings.AdditionalSlot_7 !== null && compSettings.AdditionalSlot_7 !== undefined) {
          let setts = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(compSettings.AdditionalSlot_7, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.AdditionalSlot_7 = setts;
        }
        if (compSettings.AdditionalSlot_8 !== null && compSettings.AdditionalSlot_8 !== undefined) {
          let setts = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(compSettings.AdditionalSlot_8, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
          compSettings.AdditionalSlot_8 = setts;
        }
      }
      if (obj.Fluorophore !== null && obj.Fluorophore !== undefined) {
        let newFluorophore = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(obj.Fluorophore, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
        obj.Fluorophore = newFluorophore;
      }
      if (obj.ImmersionLiquid !== null && obj.ImmersionLiquid !== undefined) {
        let newImmersionLiquid = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(obj.ImmersionLiquid, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
  ...
  static applyPreviousAppVersionModificationToArray(originalArray, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2) {
    let newArray = [];
    for (let i = 0; i < originalArray.length; i++) {
      let obj = originalArray[i];
      let newObj = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(obj, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, objSchemas, objSchemas2);
      newArray[i] = newObj;
    }
    return newArray;
  }
  ...
  applyPreviousAppVersionModificationToSetting(originalSetting) {
    ...
    if (originalSetting.Planes !== null && originalSetting.Planes !== undefined) {
      let newPlanes = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(originalSetting.Planes, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, settingsSchema, experimentalSchema);
      newSetting.Planes = newPlanes;
    }
    if (originalSetting.Channels !== null && originalSetting.Channels !== undefined) {
      let newChannels = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(originalSetting.Channels, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, settingsSchema, experimentalSchema);
      newSetting.Channels = newChannels;
    }
    if (originalSetting.TIRFSettings !== null && originalSetting.TIRFSettings !== undefined) {
      let newTIRFSettings = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(originalSetting.TIRFSettings, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, settingsSchema, experimentalSchema);
    ...
    if (originalSetting.ImagingEnvironment !== null && originalSetting.ImagingEnvironment !== undefined) {
      let newImagingEnvironment = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(originalSetting.ImagingEnvironment, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, settingsSchema, experimentalSchema);
    ...
    if (originalSetting.SamplePositioningSettings !== null && originalSetting.SamplePositioningSettings !== undefined) {
      let newSamplePositioningSettings = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(originalSetting.SamplePositioningSettings, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, settingsSchema, experimentalSchema);
    ...
    if (originalSetting.MicroscopeTableSettings !== null && originalSetting.MicroscopeTableSettings !== undefined) {
      let newMicroscopeTableSettings = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(originalSetting.MicroscopeTableSettings, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, settingsSchema, experimentalSchema);
    ...
    if (originalSetting.ObjectiveSettings !== null && originalSetting.ObjectiveSettings !== undefined) {
      ...
      let newObjectiveSettings = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(originalObjSett, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, settingsSchema, experimentalSchema);
    ...
    if (originalSetting.MicroscopeStandSettings !== null && originalSetting.MicroscopeStandSettings !== undefined) {
      let newMicroscopeStandSettings = MicroMetaAppReact.applyPreviousAppVersionModificationToObj(originalSetting.MicroscopeStandSettings, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, settingsSchema, experimentalSchema);
  ...
  applyPreviousAppVersionModificationToMicroscope(originalMicroscope) {
    ...
    if (originalMicroscope.components !== null && originalMicroscope.components !== undefined) {
      let newComponents = MicroMetaAppReact.applyPreviousAppVersionModificationToArray(originalMicroscope.components, isUpdateModelVersion, isAddModelVersion, isAddExtDomCat, fieldsToDelete, fieldsToNameChange, componentsSchema, experimentalSchema);
  ...
  createOrUseMicroscopeFromDroppedFile(resolve) {
    ...
    let adaptedSchemas = this.createAdaptedSchemas(modifiedMic.ValidationTier, standType);
    let typeDimensions = this.state.dimensions[standType];
    let microscopeSchema = adaptedSchemas[0];
    let microscopeStandSchema = adaptedSchemas[1];
    ...
    let validationStand = validate(modifiedMic.MicroscopeStand, microscopeStandSchema);
  ...
  createOrUseSettingFromDroppedFile(resolve) {
    ...
    let adaptedSchemas = this.createAdaptedSchemas(modifiedSetting.ValidationTier, this.state.standType);
    let imageSchema = adaptedSchemas[3];
    let settingsSchema = adaptedSchemas[4];
    let pixelsSchema = null;
    for (let i in settingsSchema) {
  ...
  createOrUseSetting(micLoadingOption, imgLoadingOption, settLoadingOption, micFilename, newMicroscope, settFilename, newSetting, newMetadata) {
    let promiseMicroscope = new Promise((resolve, reject) => {
      this.createOrUseMicroscope(micLoadingOption, micFilename, newMicroscope, resolve);
    });
    promiseMicroscope.then(() => {
      ...
      let modifiedCreateString = _constants.string_createFromScratch.replace("# ", "");
      ...
      if (settLoadingOption !== modifiedCreateString && settLoadingOption !== _constants.string_createFromFile) {
        ...
        if ((0, _genericUtilities.isDefined)(this.props.onLoadSetting)) {
      ...
      } else if ((0, _genericUtilities.isDefined)(this.props.onLoadSetting)) {
        this.props.onLoadSetting(-1);
      }
  ...
  onSpecialImporterConfirm(loadingOption, filename, newMicroscope) {
    let promiseMicroscope = new Promise((resolve, reject) => {
      this.createOrUseMicroscope(loadingOption, loadingOption, newMicroscope, resolve);
    });
    promiseMicroscope.then(() => {
      this.setState({
        isSpecialImporterActive: false
  ...
  onClickHome(item) {
    ...
    this.setState({
      ...
      if (this.state.is4DNPortal) {
        ...
        } else if (item === "Import" /*&& isDefined(this.props.onImportFromFile*/) {
      ...
      if ((0, _genericUtilities.isDefined)(this.props.onModeSelection)) {
  ...
  handleExportMicroscope(microscope, complete) {
    ...
    let filename = "".concat(micName, ".json");
  ...
  handleExportSetting(setting, complete) {
    ...
    let filename = "".concat(settingName, ".json");
    let contentType = "application/json;charset=utf-8;";
    var a = document.createElement("a");
    a.download = filename;
  ...
  handleExportMicroscopeImage(microscope, img /*, dataUrl*/) {
    let filename2 = "".concat(microscope.Name, ".png");
    var a = document.createElement("a");
    document.body.appendChild(a);
  ...
  render() {
    ...
    if (this.state.isCreatingNewMicroscope == null && this.state.isLoadingMicroscope == null || waitForDataLoad) {
      if (!this.state.is4DNPortal && !this.props.isMMEOpen) {
        ...
        let logoImg = url.resolve(imagesPathPNG, _constants.string_logo_img_micro_bk);
    ...
    if (this.state.is4DNPortal && (microscope === null || elementData === null || this.state.isSpecialImporterActive)) {
      ...
      if (microscope === null || this.state.isSpecialImporterActive) {
        ...
        return /*#__PURE__*/_react.default.createElement(MicroMetaAppReactContainer, {
          ...
          modeSelection: _constants.string_createFromFile
    ...
    if (this.state.isCreatingNewMicroscope /*|| this.state.isLoadingMicroscope*/ && (microscope === null || elementData === null)) {
      ...
      if (microscopeNames !== null && microscopeNames !== undefined && Object.keys(microscopeNames).length > 0) {
        if (this.props.isElectron) {
          loadingOptions.push(_constants.string_loadFromHomeFolder);
        } else {
          loadingOptions.push(_constants.string_loadFromRepository);
        }
      }
      return /*#__PURE__*/_react.default.createElement(MicroMetaAppReactContainer, {
        width: width,
    ...
    if (!this.state.isCreatingNewMicroscope && this.state.isLoadingImage && this.state.isLoadingSettings) {
      let imgLoadingOptions = [_constants.string_noImageLoad];
      if (this.props.isElectron) {
      ...
      let settLoadingOptions = [_constants.string_createFromFile];
      let settingsNames = {};
    ...
    if (!this.state.isCreatingNewMicroscope) {
      ...
      if (this.state.isViewOnly) {
        ...
        //{overlayImporter}
        let isPasteEnabled = (0, _genericUtilities.isDefined)(this.state.tmpCopyElementFromData) ? true : false;
        return /*#__PURE__*/_react.default.createElement(MicroMetaAppReactContainer, {
          width: width,
          height: height,
          forwardedRef: this.overlaysContainerRef
...
class MicroMetaAppReactContainer extends _react.default.PureComponent {
  render() {
...
const createApi = function api(context) {
  ...
  return {
    public: {
      ...
      validateMicroscope(microscope, schemas, checkForMicroscopeStand, checkForModelVersion, checkForAppVersion) {
